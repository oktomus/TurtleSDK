...
void Terrain::ui() {
  Model::ui();
  if (ImGui::Button("Randomize height")) randomize();
}
...
void Terrain::randomize(std::vector<Vertex>& pPoints, unsigned int pSlicing) {
  ...
  for (size_t i = 0; i < pPoints.size(); ++i) {
    pPoints.at(i).Position.y =
        -9 +
        25 * pn.noise(pPoints.at(i).Position.x / 20,
                      pPoints.at(i).Position.z / 20, 0.8) +
        pn.noise(pPoints.at(i).Position.x / 4, pPoints.at(i).Position.z / 4,
                 10.4) *
            0.5;
  }
}

void Terrain::calculateNormals(std::vector<Vertex>& pPoints,
  ...
  for (size_t i = 0; i < pTriangles.size(); i += 3) {
    ...
    glm::vec3 normal =
        glm::cross(pPoints.at(indB).Position - pPoints.at(indA).Position,
