...
Turtle &Turtle::getInstance() { return Turtle::instance_; }

float Turtle::getDeltaTime() const { return deltaTime_; }

int Turtle::getWinHeight() const { return winHeight_; }

int Turtle::getWinWidth() const { return winWidth_; }
...
void Turtle::init() {
...
#endif
    window_ = glfwCreateWindow(winWidth_, winHeight_, "TurtleSDK", NULL, NULL);
...

  {
    ...
    shaders_["terrain"] = std::make_shared<Shader>("turtleLib/shaders/terrain");
    ...
    shaders_["light"] = std::make_shared<Shader>(
        "turtleLib/shaders/flatBillBoardStill.vert",
  ...
  }
  {
    ...
    models_.push_back(
        std::make_shared<Model>("turtleLib/models/woodenCase/case.obj"));
    objects_.push_back(Object(models_.back()));
    objects_.back().translate_.y = 10;
    models_.push_back(
        std::make_shared<Model>("turtleLib/models/light/light.obj"));
...
void Turtle::displayFrame() {
  ...
  // STATE ASSIGN
  {
    glClearColor(clearColor_[0], clearColor_[1], clearColor_[2],
                 clearColor_[3]);
    glClearDepth(1.0);
    glEnable(GL_BLEND);
  ...
  // DRAW
  {
    ...
    for (size_t i = 0; i < 4; ++i) {
      pointLights_.at(i).setUniforms(*(shaders_["phong"].get()),
    ...
    shaders_["terrain"]->use();
    shaders_["terrain"]->setFloat("material.shininess", 32.0f);
    shaders_["terrain"]->setMat4("view", cam_->view());
    shaders_["terrain"]->setVec3("viewPos", cam_->pos);
    shaders_["terrain"]->setMat4("projection", cam_->projection());
    shaders_["terrain"]->setBool("noTexture", false);
    dirLights_.back().setUniforms(*(shaders_["terrain"].get()), "dirLight");
    for (size_t i = 0; i < 4; ++i) {
      pointLights_.at(i).setUniforms(*(shaders_["terrain"].get()),
                                     "pointLights[" + std::to_string(i) + "]");
    }
    pointLights_.back().setUniforms(*(shaders_["terrain"].get()), "spotLight");

    shaders_["terrain"]->setBool("noTexture", true);

    ground_->draw(*(shaders_["terrain"].get()));
...
void Turtle::displayLights() {
  ...
  glm::vec3 CameraRight_worldspace = {cam_->view()[0][0], cam_->view()[1][0],
                                      cam_->view()[2][0]};
  glm::vec3 CameraUp_worldspace = {cam_->view()[0][1], cam_->view()[1][1],
                                   cam_->view()[2][1]};
  ...
  // IMGUI
  {
    ...
    if (scene_window) {
      ...
      if (ImGui::CollapsingHeader("Terrain")) {
        ...
        if (ImGui::Button("Randomize Terrain")) ground_->randomize();
      }
      ...
      if (ImGui::CollapsingHeader("Lighting")) {
        ...
        for (size_t i = 0;
             i < dirLights_.size() + pointLights_.size() + spotLights_.size();
             i++)
        ...
        if (currentLight == -1)
          ...
          spotLights_.at(currentLight - dirLights_.size() - pointLights_.size())
              .ui();
        }
      }
...
void Turtle::keyCallback(GLFWwindow *, int key, int, int action, int mods) {
  ...
  if (key == GLFW_KEY_A) {
    tu.objects_.push_back(tu.objects_.at(0));
    tu.objects_.back().random();
  }
...
void Turtle::mouseButtonCallback(GLFWwindow *, int button, int action,
                                 int mods) {
  Turtle &tu = Turtle::getInstance();
